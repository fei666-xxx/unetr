...
from .nets import SPABlock, MCABlock, CABlock

from .pre_register import PreRegister
from .recursive_cascade_networks import RecursiveCascadeNetwork
...
class USMaskFormer(nn.Module):
    ...
    def __init__(
        ...
        self.fusion = TransformerBlock(hidden_size, mlp_dim, num_heads, dropout_rate)
    ...
    def init(self, gpu_id, dataset='rcc500'):
        
        if dataset=='rcc500':
            template_path = '/home/star/data1/ghf/Data/kits23/data_processed/case_00000_1/'
            print('template_path:', template_path)

            template_input = sitk.GetArrayFromImage(sitk.ReadImage(template_path + 'imaging.nii.gz'))
            template_input = torch.from_numpy(template_input.astype(np.float32)).unsqueeze(0).unsqueeze(0).cuda(gpu_id)
            
            template_seg = sitk.GetArrayFromImage(sitk.ReadImage(template_path + 'organ.nii.gz'))
            template_seg = torch.from_numpy(template_seg.astype(np.float32)).unsqueeze(0).unsqueeze(0).cuda(gpu_id)

            fixed_path = '/home/star/data1/ghf/Data/kits23/data_processed/case_00001_1/'
            fixed = sitk.GetArrayFromImage(sitk.ReadImage(fixed_path + 'imaging.nii.gz'))
            self.fixed = torch.from_numpy(fixed.astype(np.float32)).unsqueeze(0).unsqueeze(0).cuda(gpu_id)
        
        elif dataset=='brats2020':
            template_path = '/home/star/data1/ghf/Data/BraTs2020_training_data/data_nii/1/'
            print('template_path:', template_path)
            
            template_input = sitk.GetArrayFromImage(sitk.ReadImage(template_path + '3.nii.gz'))
            template_input = torch.from_numpy(template_input.astype(np.float32)).unsqueeze(0).unsqueeze(0).cuda(gpu_id)
            
            template_seg = sitk.GetArrayFromImage(sitk.ReadImage(template_path + 'organ.nii.gz'))
            template_seg = torch.from_numpy(template_seg.astype(np.float32)).unsqueeze(0).unsqueeze(0).cuda(gpu_id)

            self.fixed = template_input
        
        elif dataset=='song':
            template_path = '/home/star/data1/ghf/Data/BraTs2020_training_data/song/1-1/'
            print('template_path:', template_path)
            template_input = torch.from_numpy(np.load(template_path+'image.npy').astype(np.float32)).unsqueeze(0).unsqueeze(0).cuda()
            template_seg = torch.from_numpy(np.load(template_path+'tumor.npy').astype(np.float32)).unsqueeze(0).unsqueeze(0).cuda()
        ...
        self.build_preregister(template_input, template_seg, state_path=None, base_network='VTN')

    def load_model(self, state_dict, model):
        # load state dict
        if 'stem_state_dict' in state_dict.keys():
            model.stems.load_state_dict(state_dict['stem_state_dict'])
        else:
            model.stems.load_state_dict(state_dict)
        # if hypernet in model attribute
        if hasattr(model, 'hypernet'):
            model.hypernet.load_state_dict(state_dict['hypernet_state_dict'])
        return model

    def load_model_from_dir(self, checkpoint_dir, model):

        if checkpoint_dir[-4:]=='.pth':
        ...
        print('Building stage 1 model from', model_path)

        self.load_model(torch.load(model_path), model)
        return model_path


    def build_stage1_model(self, state_path=None, base_network='VTN'):
        """
        ...
        """
        stage1_model = RecursiveCascadeNetwork(n_cascades=3, im_size = (128,128,128), base_network=base_network, in_channels=2)
        self.load_model_from_dir(state_path, stage1_model)
        return stage1_model
    ...
    def build_preregister(self, template_input, template_seg, state_path=None, base_network='VTN'):
        ...
        """
        # state_path = state_path or '/home/star/data1/ghf/Projects/medical_reg_with_volume_preserving/logs/rcc500/VTN/May02-203504_rcc500_VTNx3/'
        # state_path = state_path or '/home/star/data1/ghf/Projects/medical_reg_with_volume_preserving/logs/rcc500/VTN/Apr28-165918_rcc500_VTNx3/'
        state_path = state_path or '/home/star/data1/ghf/Projects/medical_reg_with_volume_preserving/logs/brats2020/VTN/May09-170657_brats2020_VTNx3/'
        # state_path = '/home/star/data1/ghf/Projects/unetr/pretrained_models/VTN_d2.pth'
    ...
    def forward(self, x_in):
        ...
        # mask = x_in[:,-1]
        # mask = transforms.apply_transform(self.noise_transform, mask).unsqueeze(1)

        _, mask = self.pre_register(self.fixed, x_in[:,3].unsqueeze(1))
        ...
        for i in range(self.modality):
            ...
            # save_image(tmp[5,0,:,:,:].transpose(1,2).transpose(0,1).unsqueeze(1),'03tumor.png',nrow=8,pad_value=1)

            tmp, _ = self.blocks[0](tmp)
            output.append(tmp)
            # print(tmp.shape)
        ...
        # mask = self.gating(x_in[:,-1].unsqueeze(1)) # B N 1
        # mask = nn.functional.gumbel_softmax(mask.squeeze(), tau=0.1, hard=True).unsqueeze(-1)
        # logits = self.head(torch.mean(output*mask.repeat(1,self.modality,output.shape[-1]),dim=1))

        logits = self.head(torch.mean(output, dim=1))
