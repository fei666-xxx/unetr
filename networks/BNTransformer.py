...
class BNTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.bottlenecks = nn.Parameter(torch.zeros(1, num_BN, hidden_size))
        self.CABlocks = nn.ModuleList([CABlock(hidden_size, mlp_dim, num_heads, dropout_rate, num_BN) for _ in range(len(fusion_layer))])
    ...
    def forward(self, x_in):
        ...
        for i,layer_idx in enumerate(self.fusion_layer):
            all_tokens = []
            for j in range(self.modality):
                all_tokens.append(output[j][layer_idx-1])
            all_tokens = torch.cat(all_tokens, dim=1)
            all_tokens = torch.cat([bottlenecks,all_tokens], dim=1)
            
            bottlenecks = self.CABlocks[i](all_tokens)

        logits = self.head(torch.mean(bottlenecks,dim=1))
